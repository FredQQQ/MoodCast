...
const compatibilityForecasts={
  'bright+bright':{title:'双晴上升流',combo:'晴朗 × 晴朗',summary:'两股明亮气流同时经过，今天很容易把一个小念头变成临时出发。',adviceTitle:'留一个人负责看时间',advice:'适合临时出发，但最好留一个人记得回家的路。',base:89},
  'bright+calm':{title:'暖风缓冲带',combo:'晴朗 × 平稳',summary:'一边带来一点行动力，一边把节奏调到刚刚好。',adviceTitle:'一个提议，一个减速',advice:'一个负责提出计划，一个负责让计划不要太累。',base:91},
  'bright+complex':{title:'彩云活跃区',combo:'晴朗 × 多变',summary:'新鲜感和复杂云层同时出现，今天很适合交换最近脑子里的怪念头。',adviceTitle:'计划不要写得太满',advice:'适合分享新鲜事，不适合一次安排完整的一天。',base:84},
  'bright+low':{title:'晴雨交界线',combo:'晴朗 × 低气压',summary:'一边还有光，一边需要慢一点；靠近时不必要求天气立刻同步。',adviceTitle:'带动，但不要催晴',advice:'适合轻轻带动，不适合催对方立刻放晴。',base:78},
  'calm+calm':{title:'无风共居区',combo:'平稳 × 平稳',summary:'两套气压都很安静，今天不制造节目也能舒服地待在一起。',adviceTitle:'共享安静就很好',advice:'适合安静共处，今天没有必要制造节目效果。',base:92},
  'calm+complex':{title:'云层缓冲带',combo:'平稳 × 多变',summary:'一边提供稳定地面，一边让天空保持一点意外。',adviceTitle:'给变化留出余地',advice:'一个提供稳定气压，一个带来意外云层。',base:86},
  'calm+low':{title:'低风陪伴区',combo:'平稳 × 低气压',summary:'不需要特别热闹，也不需要马上解决什么，陪伴本身已经形成天气。',adviceTitle:'少追问，多陪一会儿',advice:'适合陪伴和散步，不必追问天气什么时候变好。',base:88},
  'complex+complex':{title:'多云实验场',combo:'多变 × 多变',summary:'两片变化丰富的云层相遇，今天可能随时产生新剧情。',adviceTitle:'准备一个备用计划',advice:'气象变化丰富，约见时最好准备两个备选计划。',base:81},
  'complex+low':{title:'间歇性静雨',combo:'多变 × 低气压',summary:'一边有许多感觉经过，一边更想降低音量，合适的距离比热闹重要。',adviceTitle:'保留一句“我在”',advice:'适合保持一点距离，也保留一句“我在”。',base:83},
  'low+low':{title:'低功耗共生区',combo:'低气压 × 低气压',summary:'两边都不想额外耗电，反而很容易理解对方为什么暂时安静。',adviceTitle:'互相免除解释义务',advice:'适合互相免除解释义务，一起低功耗运行。',base:90}
};

const periodLabels={lateNight:'深夜',morning:'上午',afternoon:'下午',evening:'晚间'};
let activeQuestions=[], current=0, chosen=[], currentClimate='mixed', selectedMode='standard', questionTotal=5, invitedClimate=null, selectedGift='sun', receivedGift=null, publicWeatherData=null, bulletinReturn='landing', currentCompatibility=null;
...
function compatibility(){
  if(!invitedClimate||!climates[invitedClimate]){$('compatibility-card').classList.add('hidden');$('invite-cta').classList.remove('hidden');currentCompatibility=null;return;}
  const theirs=climates[invitedClimate],mine=climates[currentClimate],pair=[theirs.mood,mine.mood].sort().join('+'),forecast=compatibilityForecasts[pair]||{title:'临时天气交汇区',combo:'未知气团 × 未知气团',summary:'两套天气暂时交汇，新的相处气候正在形成。',adviceTitle:'边走边观察',advice:'不必急着定义今天，先看看两股气流准备往哪里走。',base:82};
  const seed=[invitedClimate,currentClimate].sort().join('').split('').reduce((sum,char)=>sum+char.charCodeAt(0),0),score=Math.max(72,Math.min(96,forecast.base+(seed%7)-3));
  const scoreLabel=score>=91?'不说话也容易对上气压':score>=86?'今天很容易待在同一片云下':score>=81?'保留一点弹性会更合拍':'天气不同，但仍然可以互相借伞';
  currentCompatibility={...forecast,score,scoreLabel,theirs,mine};$('sky-position').before($('compatibility-card'));
  $('compatibility-their-symbol').textContent=theirs.icon;$('compatibility-their-weather').textContent=theirs.title;$('compatibility-my-symbol').textContent=mine.icon;$('compatibility-my-weather').textContent=mine.title;$('compatibility-title').textContent=forecast.title;$('compatibility-summary').textContent=forecast.summary;$('compatibility-combo').textContent=forecast.combo;$('compatibility-combo-copy').textContent=`${theirs.title}和${mine.title}正在形成同一片局部天气。`;$('compatibility-advice-title').textContent=forecast.adviceTitle;$('compatibility-text').textContent=forecast.advice;$('compatibility-score').textContent=String(score);$('compatibility-score-label').textContent=scoreLabel;$('compatibility-card').classList.remove('hidden');$('invite-cta').classList.add('hidden');
}
...
function rarityLabel(count,total){
  const rate=percentage(count,total);
  if(!total||!count)return null;
  if(count===1)return'今日首片云';
  if(total>=20&&rate<=5)return`极稀有 · ${rate}%`;
  if(total>=12&&rate<=12)return`少数气象 · ${rate}%`;
  return null;
}
function updatePersonalWeather(){
  if(!publicWeatherData||!publicWeatherData.total){$('same-weather-title').textContent='你可能是今天第一片云';$('same-weather-copy').textContent='等更多观测员抵达后，这里会显示你的真实天气占比。';$('same-weather-count').textContent='1';$('same-weather-percent').textContent='待形成';return;}
  const count=publicWeatherData.counts?.[currentClimate]||0,rate=percentage(count,publicWeatherData.total),label=rarityLabel(count,publicWeatherData.total);
  $('same-weather-title').textContent=count?`还有 ${Math.max(count-1,0)} 人和你在同一片云层`:'你的气象正在进入公共云图';
  $('same-weather-copy').textContent=label?`「${climates[currentClimate].title}」获得今日限定标记：${label}。`:`「${climates[currentClimate].title}」是今天正在形成的一股真实气团。`;
  $('same-weather-count').textContent=String(count||1);$('same-weather-percent').textContent=`${rate||'<1'}%`;
  $('rare-badge').textContent=label||'稀有气象';$('rare-badge').classList.toggle('hidden',!label);
}
function publicHeadline(data){
  if(!data?.total||!data.top?.length)return{title:'今天的天空仍在等待第一片云',lead:'完成一次观测，就能成为今日公共气象的起点。'};
  const dominant=climates[data.top[0].climate],moodLines={bright:'今天的精神高空亮度偏高。',calm:'今天的人类气压总体平稳。',complex:'今天有多股精神气团交汇。',low:'今天的云层略低，请温柔通行。'};
  return{title:`今日主导天气：${dominant.title}`,lead:`${moodLines[dominant.mood]}已有 ${data.total} 位观测员经过气象台，其中 ${data.top[0].percentage}% 正处于「${dominant.title}」。`};
}
function renderBulletin(){
  const data=publicWeatherData||{total:0,counts:{},top:[]},headline=publicHeadline(data),dominant=data.top?.[0],active=Object.entries(data.counts||{}).filter(([,count])=>count>0).sort((a,b)=>b[1]-a[1]);
  $('bulletin-date').textContent=new Intl.DateTimeFormat('zh-CN',{month:'2-digit',day:'2-digit'}).format(new Date()).replace('/',' / ');$('bulletin-symbol').textContent=dominant?climates[dominant.climate].icon:'◌';$('bulletin-title').textContent=headline.title;$('bulletin-lead').textContent=headline.lead;$('bulletin-total').textContent=String(data.total||0);$('bulletin-dominant-rate').textContent=dominant?`${dominant.percentage}%`:'—';$('bulletin-variety').textContent=String(active.length||0);
  $('bulletin-bars').innerHTML=active.length?active.slice(0,7).map(([key,count])=>`<div class="bulletin-bar"><span>${climates[key].icon} ${climates[key].title}</span><i><em style="--bulletin-width:${Math.max(percentage(count,data.total),3)}%"></em></i><b>${percentage(count,data.total)}%</b></div>`).join(''):'<p class="bulletin-empty">暂无观测。今天的天空还是一张空白气象图。</p>';
  const currentPeriod=data.periods?.[data.currentPeriod],periodName=periodLabels[data.currentPeriod]||'当前时段';
  if(currentPeriod?.top){const c=climates[currentPeriod.top.climate];$('bulletin-period-title').textContent=`${periodName} · ${c.icon} ${c.title}`;$('bulletin-period-copy').textContent=`这一时段已有 ${currentPeriod.total} 次观测，${currentPeriod.top.percentage}% 的气象信号指向「${c.title}」。`;}else{$('bulletin-period-title').textContent=`${periodName}仍在形成`;$('bulletin-period-copy').textContent='这一时段的数据还不够多，第一股明确气团随时可能出现。';}
  if(data.rarest){const c=climates[data.rarest.climate];$('bulletin-rare-title').textContent=`${c.icon} ${c.title} · ${data.rarest.percentage}%`;$('bulletin-rare-copy').textContent=`今天只有 ${data.rarest.count} 位观测员进入这片天气，是目前较少见的气象信号。`;}else{$('bulletin-rare-title').textContent='等待稀有气象';$('bulletin-rare-copy').textContent='出现中的少数天气会被记录在这里。';}
}
async function loadPublicWeather(){try{const response=await fetch('/api/weather-today',{headers:{Accept:'application/json'},cache:'no-store'});if(!response.ok)throw new Error('unavailable');const data=await response.json();publicWeatherData=data;$('public-weather-total').textContent=data.total?`今天已有 ${data.total} 人完成观测`:'今天还没有观测，你可能是第一个';if(data.top?.length){$('public-weather-bars').innerHTML=data.top.map(item=>`<div class="public-weather-row"><span>${climates[item.climate]?.icon||'◌'} ${climates[item.climate]?.title||item.climate}</span><i style="--weather-width:${Math.max(item.percentage,4)}%"></i><b>${item.percentage}%</b></div>`).join('');$('public-weather-bars').classList.remove('hidden')}else{$('public-weather-bars').classList.add('hidden')}$('public-weather-note').textContent=data.top?.[0]?publicHeadline(data).title:'只统计匿名天气类型，不记录你的具体答案。';updatePersonalWeather();renderBulletin();}catch{$('public-weather-total').textContent='公共气象台暂未连接';$('public-weather-note').textContent=location.protocol==='file:'?'部署到 Netlify 后，这里会显示真实的匿名天气统计。':'你的测试仍可正常进行，公共统计稍后再试。';renderBulletin();}}
...
function restart(){startRound();$('result').classList.add('hidden');$('quiz').classList.add('hidden');$('bulletin-page').classList.add('hidden');$('landing').classList.remove('hidden');renderHistory();window.scrollTo({top:0,behavior:'smooth'});}
...
function openBulletin(from){bulletinReturn=from;['landing','quiz','result'].forEach(id=>$(id).classList.add('hidden'));$('bulletin-page').classList.remove('hidden');renderBulletin();window.scrollTo({top:0,behavior:'smooth'});}
function closeBulletin(){$('bulletin-page').classList.add('hidden');$(bulletinReturn==='result'?'result':'landing').classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'});}
$('public-weather-more').onclick=()=>openBulletin('landing');$('result-bulletin-button').onclick=()=>openBulletin('result');$('bulletin-back').onclick=closeBulletin;
function bulletinShareText(){const headline=publicHeadline(publicWeatherData),data=publicWeatherData||{total:0};return`精神气象台 · 今日人类精神天气\n${headline.title}\n${headline.lead}\n\n今日已有 ${data.total||0} 位观测员。\n——卡卡卡卡皮巴拉`;}
...
function compatibilityShareText(){if(!currentCompatibility)return'';const c=currentCompatibility;return`我们的今日相处气象是「${c.title}」\n${c.theirs.title} × ${c.mine.title}\n今日默契 ${c.score}% · ${c.scoreLabel}\n${c.advice}\n\n——卡卡卡卡皮巴拉的精神气象台`;}
function showCompatibilityStatus(text){$('compatibility-share-status').textContent=text;$('compatibility-share-status').classList.remove('hidden');setTimeout(()=>$('compatibility-share-status').classList.add('hidden'),2200);}
$('compatibility-share').onclick=async()=>{if(!currentCompatibility)return;const url=new URL(location.href);url.search='';url.hash='';const text=compatibilityShareText();try{if(navigator.share)await navigator.share({title:'我们的今日相处气象',text,url:url.toString()});else{await navigator.clipboard.writeText(`${text}\n\n也来观测：${url}`);showCompatibilityStatus('相处气象和网站链接已复制');}}catch{try{await navigator.clipboard.writeText(`${text}\n\n也来观测：${url}`);showCompatibilityStatus('相处气象和网站链接已复制');}catch{showCompatibilityStatus('暂时无法分享，请保存图片或截图');}}};
$('compatibility-save').onclick=async()=>{if(!currentCompatibility)return;const b=$('compatibility-save'),card=$('compatibility-card');b.textContent='正在生成…';card.classList.add('capturing');try{if(!window.html2canvas)throw new Error('library unavailable');const canvas=await html2canvas(card,{scale:2,backgroundColor:null,useCORS:true});const link=document.createElement('a');link.download=`Moodcast-相处气象-${currentCompatibility.title}.png`;link.href=canvas.toDataURL('image/png');link.click();b.textContent='已保存 ✓';}catch{b.textContent='保存失败，请截图';}finally{card.classList.remove('capturing')}setTimeout(()=>b.textContent='保存相处气象图',1800);};
