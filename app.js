...
function showResult(){currentClimate=selectClimate(score());$('quiz').classList.add('hidden');$('result').classList.remove('hidden');$('card-date').textContent=new Intl.DateTimeFormat('zh-CN',{month:'2-digit',day:'2-digit'}).format(new Date()).replace('/',' / ');fillResult();compatibility();saveHistory();window.scrollTo({top:0,behavior:'smooth'});}
...
document.querySelectorAll('[data-feedback]').forEach(button=>button.onclick=()=>{$('accuracy-check').querySelector('div').classList.add('hidden');$('feedback-thanks').classList.remove('hidden');});
...
renderHistory();renderAtlas();restoreReverseForecast();
